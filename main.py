...
def readFromImage(file):
    with open(file, "rb") as f:
        raw = f.read()
    f.close()

    raw.translate(bytes(range(255,-1,-1)))

    img = Image.frombytes("RGB", (800, 480), raw)
...
def camera():
    ...
    with pc.PiCamera() as cam:
        ...
        bttn.wait_for_press()
        cam.stop_preview()
        cam.capture("tmp.rgb", format = "rgb")
...
def wifiWrapper(pssw, Cell):
    def tmp():
        s = wifi.Scheme.for_cell("wlan0", Cell.ssid, Cell, pssw.get())
        s.save()
        s.activate()
...
def getSettingsFrame():
    ...
    tk.Label(out, text="WiFi", font='Helvetica').grid(row=1, column=0)

    wifiFrame = tk.Frame(out)
    ssids = []

    w = list(wifi.Cell.all("wlan0"))
    for i in range(len(w)):
        if w[i].ssid in ssids:
            continue

        ssids.append(w[i].ssid)

        f = Frame(wifiFrame)
        tk.Label(f, text=w[i].ssid, font=monoFont).grid(row=0,column=0)
        pssw = tk.StringVar()
        tk.Entry(f, textvariable = pssw).grid(row=0,column=1)
        tk.Button(f, text="connect", font=monoFont, command=wifiWrapper(pssw,w[i])).grid(row=0,column=2)

        if len(ssids) >= 5:
            break
    wifiFrame.grid(row=1,column=1)
...
photo_set = Image.open('root/settingsIcon.png')
photo_set = photo_set.resize((40, 40))
...
photo_cam = Image.open('root/cameraIcon.png')
photo_cam = photo_cam.resize((48, 40))
img_cam = ImageTk.PhotoImage(photo_cam)
cameraButton = tk.Button(topBar, text="camera", image=img_cam, font=monoFont, command = camera)
